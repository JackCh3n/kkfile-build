...
| 分支 | 版本线 | JDK | 说明 |
| --- | --- | --- | --- |
| **`main`（本分支）** | 5.x（当前 5.0.2） | 21 | Spring Boot 3.5，有 actuator（`/actuator/health`） |
| `4.x` | 4.4.0 / 4.3.0 / 4.2.1 / 4.2.0 / 4.1.0 | 8 | Spring Boot 2.4.2，**无 actuator**（探活用 `/`、4.1.0 用 `/index`） |

两条版本线的构建体系差异较大（JDK、启动探活方式、发布标签），所以分开维护：

* 本分支（5.x）滚动发布到 Release `latest`，镜像打 `latest` / `<版本>` / `v<版本>`
* `4.x` 分支滚动发布到 `latest-4.x`，镜像打 `latest-4.x` / `<版本>`，**不会覆盖 5.x 的 `latest`**
* 两个分支都已实际跑通「编译 → 打包 → 发布 → 多架构镜像 → 冒烟测试」；4.x 的
  4.1.0/4.2.0/4.2.1/4.3.0/4.4.0 五个版本均有独立 Release，详见 4.x 分支的 README
